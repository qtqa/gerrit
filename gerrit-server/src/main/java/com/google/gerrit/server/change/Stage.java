...
import com.google.gerrit.server.change.Stage.Input;
...
public class Stage implements RestModifyView<RevisionResource, Input> {
  ...
  @Inject
  Stage(Provider<ReviewDb> dbProvider,
  ...
  public Output apply(RevisionResource rsrc, Input input) throws AuthException,
    ...
    if (!control.canStage()) {
      throw new AuthException("staging not permitted");
    } else if (change.getStatus() != Change.Status.NEW) {
    ...
    final Branch.NameKey stagingBranch = StagingUtil.getStagingBranch(change.getDest());
    if (!ProjectUtil.branchExists(repoManager, stagingBranch)) {
      Repository git = repoManager.openRepository(control.getProject().getNameKey());
      try {
    ...
    if (input.waitForMerge) {
      mergeQueue.merge(stagingBranch);
      change = dbProvider.get().changes().get(change.getId());
    } else {
      mergeQueue.schedule(stagingBranch);
    }
    ...
    switch (change.getStatus()) {
      case STAGING:
        return new Output(Status.STAGING, change);
      case STAGED:
        return new Output(Status.STAGED, change);
      case NEW:
  ...
  public Change stage(RevisionResource rsrc, IdentifiedUser caller)
      throws OrmException {
  ...
  private void approve(PatchSet rev, IdentifiedUser caller, Timestamp timestamp)
    ...
    stgn.setValue((short) 1);
    stgn.setGranted(timestamp);
    dbProvider.get().patchSetApprovals().upsert(Collections.singleton(stgn));
  }

  private void checkSubmitRule(RevisionResource rsrc)
      throws ResourceConflictException {
    // Submit rules are used also for staging
    ...
    if (ok.isPresent()) {
      ...
      throw new IllegalStateException(String.format(
          "ChangeControl.canStage returned empty list for %s in %s",
  ...
  public static class CurrentRevision implements
    ...
    public Object apply(ChangeResource rsrc, Input input) throws AuthException,
      ...
      Output out = stage.apply(new RevisionResource(rsrc, ps), input);
